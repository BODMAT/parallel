export class Semaphore {
    ...
    acquire(): void {
        while (true) {
            let val = Atomics.load(this.syncArr, this.idx);
            while (val > 0) {
                if (Atomics.compareExchange(this.syncArr, this.idx, val, val - 1) === val) {
                    return;
                }
                val = Atomics.load(this.syncArr, this.idx);
            }
    ...
    release(): void {
        Atomics.add(this.syncArr, this.idx, 1);
        Atomics.notify(this.syncArr, this.idx, 1);
    }
}